...
| 2 | **Use of approved open licenses** | ✅ | Source code under **GNU AGPL-3.0** ([`LICENSE`](../LICENSE)); sample/derived data under **CC-BY-4.0**. The DEA module uses the open-source PuLP/CBC solver, not the proprietary Gurobi (see header of [`scripts/dea.py`](../scripts/dea.py)). |
...
| 4 | **Platform independence** | ✅ | Pure-Python pipeline (3.11+) with only OSI-approved open-source dependencies ([`requirements.txt`](../requirements.txt)). No mandatory proprietary components or hosted services. |
...
| 6 | **Mechanism for extracting data** | ✅ | All inputs and outputs are non-proprietary, machine-readable formats — CSV (UTF-8), GeoJSON (EPSG:4326), TOML, Markdown. Output schema is described in [`data/data_dictionary.csv`](../data/data_dictionary.csv). |
...
- **Geography neutrality.** The core (`config/`, `scripts/`, `data/`) is
  geography-neutral and runs out of the box on a synthetic sample. All
  India-specific assumptions (government finance schemes, fiscal calendar,
  administrative coding, named data sources) are isolated under
  [`contrib/india/`](../contrib/india/).
- **Reproducibility.** The sample dataset is generated deterministically by
  ...
  full pipeline can be reproduced end-to-end from a fresh clone.
